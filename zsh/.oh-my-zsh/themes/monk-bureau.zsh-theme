...
# Python Virtual Environment Display using Capital Euler 'ℰ'
bureau_virtualenv() {
  if [[ -n "$VIRTUAL_ENV" ]]; then
    local symbol="ℰ"
    local env_name="$(basename "$VIRTUAL_ENV")"
    
    # If the environment is named .venv or venv, use the parent directory's name
    if [[ "$env_name" == ".venv" || "$env_name" == "venv" ]]; then
      env_name="$(basename "$(dirname "$VIRTUAL_ENV")")"
    fi

    echo "%{$fg_bold[green]%}${symbol}%{$fg_bold[yellow]%}::%{$fg_bold[cyan]%}${env_name}%{$reset_color%} "
  fi
}
...
setopt prompt_subst
PROMPT='$(bureau_virtualenv)> $_LIBERTY '
